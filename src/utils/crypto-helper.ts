...
export function generateSigningKey(): Promise<CryptoKeyPair> {
    return subtle.generateKey(signingKeyAlgorithm, true, ["sign", "verify"]);
}
...
export function importSigningKey(cryptoKey: string): Promise<CryptoKey> {
    ...
    return subtle.importKey("jwk", jwk, signingKeyAlgorithm, true, jwk.key_ops);
}
...
export function generateEncryptionKey(): Promise<CryptoKey> {
    return subtle.generateKey(encryptionAlgorithm, true, [
        "encrypt",
        "decrypt",
    ]);
}
